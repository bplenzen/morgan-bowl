...
all_play_season as (
    select
        ...
        round(sum(all_play_wins)::double / sum(all_play_games), 3) as all_play_win_pct
...
expected_wins_calc as (
    select
        ...
        round(all_play_win_pct * (select count(distinct week) from weekly_matchups), 2) as expected_wins
...
opponent_performance as (
    select
        ...
        avg(opp_all.points) over (partition by m.week, m.opponent_roster_id) as opponent_avg_points
    from {{ ref('fct_matchups') }} as m
    left join weekly_matchups as opp_all
        on opp_all.roster_id = m.opponent_roster_id
),
...
schedule_luck as (
    select
        ...
        round(avg(opponent_points - opponent_avg_points), 2) as schedule_luck_index
...
close_games as (
    select
        ...
        sum(case when abs(points - opponent_points) < 10 then 1 else 0 end) as total_close_games
...
final as (
    select
        ...
        round(
            50 + 
            (ar.actual_wins - ew.expected_wins) * 10 +  -- ±10 per win over/under expected
    ...
    left join expected_wins_calc as ew using (roster_id, manager_name)
...
select
    ...
    case
        ...
        when schedule_luck_index > -2 then 'Average Schedule ⚖️'
    ...
    case
        ...
        when composite_luck_score >= 45 then '⚖️ Fair'
