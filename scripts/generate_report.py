...
def generate_weekly_report(week: int) -> str:
    ...
    # The ? placeholder is replaced with the week parameter safely
...
def send_email_report(week: int, recipients: list[str]):
    ...
    if not sender or not password:
        print(
            "⚠️  Email credentials not set. Set EMAIL_SENDER and EMAIL_PASSWORD environment variables."
        )
        return
...
def post_to_slack(week: int, webhook_url: str = None):
    ...
    if not webhook_url:
        print(
            "⚠️  Slack webhook not configured. Set SLACK_WEBHOOK_URL environment variable."
        )
        return
...
if __name__ == "__main__":
    ...
    latest_week = conn.execute(
        "SELECT MAX(week) FROM main_analytics.fct_matchups"
    ).fetchone()[0]
