...
if page == "📊 Standings":
    ...
    def create_bracket_figure(
        bracket_teams, playoff_scores_df, title, is_championship=True
    ):
        """Create a visual tournament bracket using Plotly"""
        fig = go.Figure()
        ...
        if is_championship:
            # Championship bracket (6 teams, 2 byes)
            # Round 1 positions (week 15) - only 2 matchups
            round1_y_positions = [100, 300]  # 4v5 top, 3v6 bottom

            # Round 1: 4v5
            team4 = bracket_teams[3]  # Seed 4
            team5 = bracket_teams[4]  # Seed 5
            score_4 = get_score(team4["manager_name"], 15)
            score_5 = get_score(team5["manager_name"], 15)

            # Matchup 1: 4v5
            y_pos = round1_y_positions[0]
            fig.add_shape(
                type="rect",
                x0=0,
                y0=y_pos,
                x1=box_width,
                y1=y_pos + box_height,
                line=dict(color="green" if score_4 and score_4 > score_5 else "gray"),
                fillcolor="lightgreen" if score_4 and score_4 > score_5 else "white",
            )
            fig.add_annotation(
                x=box_width / 2,
                y=y_pos + box_height / 2,
                text=f"#{team4['seed']} {team4['manager_name']}<br>{score_4:.1f if score_4 else 'TBD'}",
                showarrow=False,
                font=dict(
                    size=10,
                    color="darkgreen" if score_4 and score_4 > score_5 else "black",
                ),
            )
            ...
            fig.add_shape(
                type="rect",
                x0=0,
                y0=y_pos - box_height - 5,
                x1=box_width,
                y1=y_pos - 5,
                line=dict(color="green" if score_5 and score_5 > score_4 else "gray"),
                fillcolor="lightgreen" if score_5 and score_5 > score_4 else "white",
            )
            fig.add_annotation(
                x=box_width / 2,
                y=y_pos - box_height / 2 - 5,
                text=f"#{team5['seed']} {team5['manager_name']}<br>{score_5:.1f if score_5 else 'TBD'}",
                showarrow=False,
                font=dict(
                    size=10,
                    color="darkgreen" if score_5 and score_5 > score_4 else "black",
                ),
            )

            # Round 1: 3v6
            team3 = bracket_teams[2]  # Seed 3
            team6 = bracket_teams[5]  # Seed 6
            score_3 = get_score(team3["manager_name"], 15)
            score_6 = get_score(team6["manager_name"], 15)

            y_pos = round1_y_positions[1]
            fig.add_shape(
                type="rect",
                x0=0,
                y0=y_pos,
                x1=box_width,
                y1=y_pos + box_height,
                line=dict(color="green" if score_3 and score_3 > score_6 else "gray"),
                fillcolor="lightgreen" if score_3 and score_3 > score_6 else "white",
            )
            fig.add_annotation(
                x=box_width / 2,
                y=y_pos + box_height / 2,
                text=f"#{team3['seed']} {team3['manager_name']}<br>{score_3:.1f if score_3 else 'TBD'}",
                showarrow=False,
                font=dict(
                    size=10,
                    color="darkgreen" if score_3 and score_3 > score_6 else "black",
                ),
            )
            ...
            fig.add_shape(
                ...
                x1=box_width,
                y1=y_pos - 5,
                line=dict(color="green" if score_6 and score_6 > score_3 else "gray"),
                fillcolor="lightgreen" if score_6 and score_6 > score_3 else "white",
            )
            fig.add_annotation(
                x=box_width / 2,
                y=y_pos - box_height / 2 - 5,
                text=f"#{team6['seed']} {team6['manager_name']}<br>{score_6:.1f if score_6 else 'TBD'}",
                showarrow=False,
                font=dict(
                    size=10,
                    color="darkgreen" if score_6 and score_6 > score_3 else "black",
                ),
            )
            ...
            winner_45 = (
                team4["manager_name"]
                if score_4 and score_5 and score_4 > score_5
                else (
                    team5["manager_name"]
                    if score_5 and score_4 and score_5 > score_4
                    else None
                )
            )
            winner_36 = (
                team3["manager_name"]
                if score_3 and score_6 and score_3 > score_6
                else (
                    team6["manager_name"]
                    if score_6 and score_3 and score_6 > score_3
                    else None
                )
            )

            # Semifinals (Week 16)
            team1 = bracket_teams[0]  # #1 seed
            team2 = bracket_teams[1]  # #2 seed

            semi_y_positions = [80, 280]

            # Semi 1: #1 vs winner of 4v5
            y_pos = semi_y_positions[0]
            score_1 = get_score(team1["manager_name"], 16)
            score_w45 = get_score(winner_45, 16) if winner_45 else None

            fig.add_shape(
                type="rect",
                x0=round_spacing,
                y0=y_pos,
                x1=round_spacing + box_width,
                y1=y_pos + box_height,
                line=dict(
                    color=(
                        "green"
                        if score_1 and score_w45 and score_1 > score_w45
                        else "gray"
                    )
                ),
                fillcolor=(
            ...
            fig.add_annotation(
                x=round_spacing + box_width / 2,
                y=y_pos + box_height / 2,
                text=f"#{team1['seed']} {team1['manager_name']}<br>{score_1:.1f if score_1 else 'TBD'}",
                showarrow=False,
                font=dict(
                    size=10,
                    color=(
            ...
            fig.add_shape(
                ...
                line=dict(
                    color=(
                        "green"
                        if score_w45 and score_1 and score_w45 > score_1
                        else "gray"
                    )
                ),
                fillcolor=(
            ...
            fig.add_annotation(
                x=round_spacing + box_width / 2,
                y=y_pos - box_height / 2 - 5,
                text=f"{winner_45 if winner_45 else 'Winner 4/5'}<br>{score_w45:.1f if score_w45 else 'TBD'}",
                showarrow=False,
                font=dict(
                    size=10,
                    color=(
            ...
            y_pos = semi_y_positions[1]
            score_2 = get_score(team2["manager_name"], 16)
            score_w36 = get_score(winner_36, 16) if winner_36 else None
            ...
            fig.add_shape(
                ...
                line=dict(
                    color=(
                        "green"
                        if score_2 and score_w36 and score_2 > score_w36
                        else "gray"
                    )
                ),
                fillcolor=(
            ...
            fig.add_annotation(
                x=round_spacing + box_width / 2,
                y=y_pos + box_height / 2,
                text=f"#{team2['seed']} {team2['manager_name']}<br>{score_2:.1f if score_2 else 'TBD'}",
                showarrow=False,
                font=dict(
                    size=10,
                    color=(
            ...
            fig.add_shape(
                ...
                line=dict(
                    color=(
                        "green"
                        if score_w36 and score_2 and score_w36 > score_2
                        else "gray"
                    )
                ),
                fillcolor=(
            ...
            fig.add_annotation(
                x=round_spacing + box_width / 2,
                y=y_pos - box_height / 2 - 5,
                text=f"{winner_36 if winner_36 else 'Winner 3/6'}<br>{score_w36:.1f if score_w36 else 'TBD'}",
                showarrow=False,
                font=dict(
                    size=10,
                    color=(
            ...
            finalist_1 = (
                ...
                if score_1 and score_w45 and score_1 > score_w45
                    ...
                    winner_45 if score_w45 and score_1 and score_w45 > score_1 else None
                )
            )
            finalist_2 = (
                team2["manager_name"]
                if score_2 and score_w36 and score_2 > score_w36
                    ...
                    winner_36 if score_w36 and score_2 and score_w36 > score_2 else None
                )
            )

            y_pos = 180
            score_f1 = get_score(finalist_1, 17) if finalist_1 else None
            score_f2 = get_score(finalist_2, 17) if finalist_2 else None

            fig.add_shape(
                type="rect",
                x0=round_spacing * 2,
                y0=y_pos,
                x1=round_spacing * 2 + box_width,
                y1=y_pos + box_height,
                line=dict(
                    color=(
                        "gold"
                        if score_f1 and score_f2 and score_f1 > score_f2
                        else "gray"
                    )
                ),
                fillcolor=(
                    "gold" if score_f1 and score_f2 and score_f1 > score_f2 else "white"
                ),
            )
            fig.add_annotation(
                x=round_spacing * 2 + box_width / 2,
                y=y_pos + box_height / 2,
                text=f"{finalist_1 if finalist_1 else 'Finalist 1'}<br>{score_f1:.1f if score_f1 else 'TBD'} {'🏆' if score_f1 and score_f2 and score_f1 > score_f2 else ''}",
                showarrow=False,
                font=dict(
                    size=10,
                    color=(
            ...
            fig.add_shape(
                ...
                line=dict(
                    color=(
                        "gold"
                        if score_f2 and score_f1 and score_f2 > score_f1
                        else "gray"
                    )
                ),
                fillcolor=(
                    "gold" if score_f2 and score_f1 and score_f2 > score_f1 else "white"
                ),
            )
            fig.add_annotation(
                x=round_spacing * 2 + box_width / 2,
                y=y_pos - box_height / 2 - 5,
                text=f"{finalist_2 if finalist_2 else 'Finalist 2'}<br>{score_f2:.1f if score_f2 else 'TBD'} {'🏆' if score_f2 and score_f1 and score_f2 > score_f1 else ''}",
                showarrow=False,
                font=dict(
                    size=10,
                    color=(
            ...
            # Round 1 to Semifinals
            fig.add_shape(
                type="line",
                x0=box_width,
                y0=round1_y_positions[0] - box_height / 2 - 2.5,
                x1=round_spacing,
                y1=semi_y_positions[0] - box_height / 2 - 2.5,
                line=dict(color="lightgray", width=2),
            )
            fig.add_shape(
                type="line",
                x0=box_width,
                y0=round1_y_positions[1] - box_height / 2 - 2.5,
                x1=round_spacing,
                y1=semi_y_positions[1] - box_height / 2 - 2.5,
                line=dict(color="lightgray", width=2),
            )
            ...
            fig.add_shape(
                type="line",
                x0=round_spacing + box_width,
                y0=semi_y_positions[0] - box_height / 2 - 2.5,
                x1=round_spacing * 2,
                y1=y_pos - box_height / 2 - 2.5,
                line=dict(color="lightgray", width=2),
            )
            fig.add_shape(
                type="line",
                x0=round_spacing + box_width,
                y0=semi_y_positions[1] - box_height / 2 - 2.5,
                x1=round_spacing * 2,
                y1=y_pos - box_height / 2 - 2.5,
                line=dict(color="lightgray", width=2),
            )
            ...
            fig.add_annotation(
                x=round_spacing + box_width / 2,
                y=400,
                text="<b>Semifinals</b><br>(Week 16)",
                showarrow=False,
                font=dict(size=12),
            )
            fig.add_annotation(
                x=round_spacing * 2 + box_width / 2,
                y=400,
                text="<b>Championship</b><br>(Week 17)",
                showarrow=False,
                font=dict(size=12),
            )
        ...
        fig.update_layout(
            title=title,
            xaxis=dict(visible=False, range=[-20, round_spacing * 2 + box_width + 20]),
    ...
    if not standings_df.empty:
        # Convert standings to list of dicts for easier bracket building
        teams_list = standings_df.to_dict("records")

        # Championship bracket
        championship_teams = teams_list[:6]  # Top 6 seeds
        fig_championship = create_bracket_figure(
            championship_teams,
            playoff_scores_df,
        ...
        st.markdown("---")
        st.caption(
            "Visual bracket updates automatically as playoff games are scored. "
            "Winners highlighted in green/gold and advance to next round."
        )
